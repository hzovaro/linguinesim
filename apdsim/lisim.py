...
def luckyImage(im, psf, tt, noise_frame, scale_factor, master_frame, etc_input):
	...
	"""
	# Convolve with PSF.
	im = convolvePSF(im, psf)		
	# Resize to detector.
	im = resizeImageToDetector(image_raw = im, source_plate_scale_as = 1, dest_plate_scale_as = scale_factor)
	# Add tip and tilt.
	im = addTipTilt_single(image = im, tt_idxs = tt)[0]
	# Convert to counts.
	im = expectedCount2count(im, t_exp = etc_input['t_exp'])
	# Add noise. 
	im += noise_frame
