...
def thermalEmissionIntensity(		
	...
	"""
		Given a blackbody with emissivity eps and temperature T, find the total 
		irradiance over an interval [wavelength_min, wavelength_max] incident 
		upon a pupil/emitter system with etendue A * Omega and efficiency eta. 

		I is in units of photons per second (per area A) if eta does not include 
		the QE.

		I is in units of electrons per second (per area A) if eta does include 
		the QE.
	"""
...
def surface_brightness2countRate(mu, A_tel, 
	...
	if (band == None and (wavelength_m == None or bandwidth_m == None)) or (band != None and (wavelength_m != None or bandwidth_m != None)):
		raise UserWarning('ERROR: you must specify either a band (J, H or K) \
			OR a wavelength_m and bandwidth!')
		return
	...
	if magnitude_system.lower() == 'ab':
		...
		if band != None:
			...
			raise UserWarning('ERROR: if Vega magnitudes are specified you must\
				 also specify the band!')
	else:
		zeropoint = 0
...
def surface_brightness2flux(mu, 
	...
	"""" 
		Convert a given surface brightness (expressed in magnitudes/arcsec^2) 
		OR magnitude (in magnitudes) to spectral radiance units in both per unit 
		frequency and per unit wavelength_m.

		The flux values are returned in both CGS and SI units.

		If a wavelength_m is not specified then only the spectral radiance per 
		unit 
...
def Fnucgs2flux(F_nu_cgs,
	...
	"""
		Convert a given flux frequency density specified in cgs units 
		(ergs/s/Hz/cm^-2) to cgs flux wavelength density units 
		(ergs/s/Angstrom/cm^-2). Also returns these two flux values in SI units.
	"""
...
def photonRate2countRate(Sigma_photons, A_tel, 
	...
	"""
		Convert a flux given in units of photons/s/m^2/arcsec^2 (or in units of 
		photons/s/m^2 depending on the units of Sigma_photons) into detector 
		counts given a telescope collecting area (A_tel), detector plate scale 
		(arcsec/pixel), throughput (tau), quantum efficiency (qe) and gain 
		(gain). 

		If the input is not specified per square arcsec, then the 
		plate_scale_as_px should be left blank.
	"""
...
def flux2countRate(F, 
	...
	""" 
		Return the electron count (e/pixel/s) from a source with a given  flux F 
		imaged through a system with collecting area A_tel, throughput tau, 
		quantum efficiency qe, internal gain gain and a detector plate scale 
		(plate_scale_as) in arcsec/pixel 
	"""
	...
	if band != None:
		...
		print('ERROR: you must specify either a band (J, H or K) OR a \
			wavelength_m and bandwidth!')
		return

	Sigma_photons = flux2photonRate(
		F=F, 
		wavelength_m=wavelength_m, 
		bandwidth_m=bandwidth_m)
...
def expectedCount2count(arg, 
	...
	"""
		...
		rate (in photons/s) to a 'truth' count assuming a Poisson distribution.
		If t_exp is not specified, it is assumed that the input argument is 
		given in units of photons. 

		If t_exp is specified, it is assumed that the input argument is given 
		in units of photons/s, in which case the expected count is arg * t_exp.

		The saturation level of the detector may also be specified if desired. 
	...
	if len(expectedCount[expectedCount<0].flatten()) > 0:
		print("WARNING: input image has {:d} negative values! Clamping negative\
