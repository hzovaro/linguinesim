...
def simulateSatellite(dim_m,
	...
	F_nu_cgs = F_lambda_cgs * (FILTER_BANDS_M[band][0]**2) / scipy.constants.c 		# ergs/s/cm^2/Hz/arcsec^2
	...
	E_phot = scipy.constants.h * scipy.constants.c / FILTER_BANDS_M[band][0]	# Photon energy (W)
	...
	# Saving to file
	if im_out_fname:
		imutils.exportFitsFile(image_in_array=im_raw, fname=fname, otherHeaderData={
			'dim_m' : dim_m,
...
def solarFlux(band):
	...
	"""
	E_phot = scipy.constants.h * constants. c / FILTER_BANDS_M[band][0]	# We must multiply the output by E_phot because thermalEmissionIntensity returns the irradiance in photons/s, not in W
	return etcutils.thermalEmissionIntensity(T=T_SUN_K,
